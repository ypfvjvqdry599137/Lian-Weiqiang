...
def get_supplier_settlement():
    ...
    for order in orders:
        ...
        orders_output.append({
            ...
            'zone_id': order.order.zone_id if order.order else None,
            'zone_name': order.order.zone.zone_name if (order.order and order.order.zone) else None,
            'station_id': order.order.zone.station.id if (order.order and order.order.zone and order.order.zone.station) else None,
            'station_name': order.order.zone.station.station_name if (order.order and order.order.zone and order.order.zone.station) else None,
            'status': order.status,
            'status_text': status_text,
